...
ggplot(perplexity_results, aes(x = as.factor(NumTopics), y = Perplexity, fill = Tema)) +
  geom_bar(stat = "identity", position = position_dodge()) +
  labs(title = "Perplexity vrednosti za svaku kategoriju i broj tema", x = "Broj Tema", y = "Perplexity") +
  facet_wrap(~ Tema, scales = "free_x") +
  theme_minimal() +
...
ggplot(logLik_results, aes(x = as.factor(NumTopics), y = LogLikelihood, fill = Tema)) +
  geom_bar(stat = "identity", position = position_dodge()) +
  labs(title = "Log-Likelihood vrednosti za svaku kategoriju i broj tema", x = "Broj Tema", y = "Log-Likelihood") +
  facet_wrap(~ Tema, scales = "free_x") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
