...
stopwords_sr <- c(
  ...
  "bila", "bili", "bilo", "će", "hoće", "biće", "mogu", "može", "jaja", "ne", "da", "li", "već", "jer", "pa", "takođe",
  "gde", "tamo", "ovde", "iz", "preko", "sve", "svaki", "sva", "svako", "svi", "svakog", "svega", "sebe", "sebi", "-",         "httpsphotosakfbcdnnethphotosaksnchssncnjpg","httpasphotosakfbcdnnethphotosaksncnjpg"," httpasphotosakfbcdnnethphotosakashnjpg","httpsphotosakfbcdnnethphotosakashhsashnjpg",
  "sebe", "ovaj", "onaj", "ta", "ovo", "ono", "možda", "čak", "ipak", "nekad", "uvek", "nikad", "sad", "sada","jebi", "–",
  ...
  "jos", "moze","vec", "vise","sranje", "the", "jedan", "httpwwwimdbcomtitlett", "smo", "neke","nesto", "mora","koliko", "govna", "evo",
  "you", "neku", "jebe", "for", "and", "zna", "jebem", "haha","god", "bre", "that", "not", "ću", "nisu", "nista", "mozda",
...
process_topic <- function(data, topic_name, num_topics = 5) {  
  ...
  ggsave(filename = paste0(topic_name, "_top_terms5.png"), plot = p, width = 10, height = 8)
  
  # Prikazivanje dokumenata i pripadajućih tema
  doc_topics <- tidy(lda_model, matrix = "gamma")
  
  # Usklađivanje dokumenata i tema
  doc_topics <- doc_topics %>%
    group_by(document) %>%
    slice_max(gamma, n = 1) %>%
    ungroup()
  
  # Prilagođavanje redosleda redova između `data` i `doc_topics`
  data$topic <- doc_topics$topic[match(1:nrow(data), doc_topics$document)]
  
  # Snimanje rezultata u CSV fajl
  write_csv(data, paste0(topic_name, "_sa_temama5.csv"))
}
...
# Obrada podataka
process_topic(sport, "sport", 5)
process_topic(umetnost, "umetnost", 5)
process_topic(tehnologija, "tehnologija", 5)
process_topic(priroda, "priroda", 5)
process_topic(drustvo, "drustvo", 5)
